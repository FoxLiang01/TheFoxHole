<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['business:articleTag:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['business:articleTag:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['business:articleTag:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['business:articleTag:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="articleTagList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="tag的id" align="center" prop="tagId" />
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        tagId: [
          { required: true, message: "tag的id不能为空", trigger: "blur" }
        ]
      }
    };
  },
  ...
  methods: {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加文章与标题的关联";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const articleId = row.articleId || this.ids
      getArticleTag(articleId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改文章与标题的关联";
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除文章与标题的关联编号为"' + articleIds + '"的数据项？').then(function() {
        return delArticleTag(articleIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('business/articleTag/export', {
        ...this.queryParams
